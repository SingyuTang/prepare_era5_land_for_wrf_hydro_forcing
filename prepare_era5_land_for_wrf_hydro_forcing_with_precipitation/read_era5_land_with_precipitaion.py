...
for i, item in enumerate(era5_list):
    ...
    for j, jtem in enumerate(every_data.time): # disaggreated
        ...
        # Convert TP into RAINRATE
        hr_dataset.variables['tp'][:] = hr_dataset.variables['tp'][:] * 1000 / 3600 # m unit to mm/s
        hr_dataset = hr_dataset.rename_vars({'tp': 'rp'})  # replace fields name tp to rp
        hr_dataset.variables['rp'].attrs['units'] = 'mm s-1'  # update unit name
        hr_dataset.variables['rp'].attrs['long_name'] = 'Precipitation rate'
        ...
        hr_dataset.to_netcdf(outputname, encoding={'rp': {'dtype': 'float32', '_FillValue': -32767, 'missing_value': -32767,},
                                                   'ssrd': {'dtype': 'float32', '_FillValue': -32767, 'missing_value': -32767,},
